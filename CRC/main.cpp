...
static constexpr bool print_tables = false;
...
int main() {
	...
	constexpr size_t bytes = 900 * 1024;
	uint8_t* M = new uint8_t[bytes];
	...
	for (const TestItem& item : items)
	{
		...
		if (item.m_hasPrev) {
			for (size_t i = 0; i < item.m_runs; ++i) {
				result = item.m_fPrev(M, bytes, 0);
			}
		}
		else {
			for (size_t i = 0; i < item.m_runs; ++i) {
				result = item.m_fNoPrev(M, bytes);
			}
		}
		...
		printf(" %s | 0x%08x | %.1f MB/s\n", item.m_name, result, bytes / ns * 1e3);
	}
